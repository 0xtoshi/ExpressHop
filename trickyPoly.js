...
(async() => {
    ...
    async function getAndInsert(startTime, endTime)
    {
        const GraphQL = await axios({
            url: 'https://api.thegraph.com/subgraphs/name/hop-protocol/hop-polygon',
            method: 'post',
            data: {
              query: `
          ...
          for(let data of GraphQL.data.data.transferSents)
          {
             ...
             var datane = {
              ...
              try{
              knex('transferSentPOLY').insert(datane).then(result => {})
              }catch(e){console.log('Error Maybe Duplicate')};
          }
