...
abiDecoder.addABI(ABI);
  ...
  .then(async result => {
      ...
      {
          if(data.address_from !== data.recipient && data.destinationChainId == 10)
          {
  ...
  async function TrackPoly(root_address)
  {
    ...
    //console.log(PolygonScanAPI.data.result);
     ...
     if(FinalList.length > 1){
        fs.appendFileSync('suspectTrackArb.txt', root_address+'\n');
     for(let finaladdress of FinalList){
         ...
         if(Hop.data.data.hopUserTokens > 0)
         {
             ...
             fs.appendFileSync('suspectTrackArb.txt', `-] ${finaladdress.address}\n`);
         }
     }
    }
  }